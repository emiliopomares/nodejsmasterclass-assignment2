	...
	var storedUser = storage.retrieveObject('users', email, function(operationResult) {
		...
		// user had been previously registered in storage
			...
			if(storedUser.password!==passwordhash.hash(password)) {
				callback(operation.success(false, 'Wrong credentials'));
			}
			else callback(operation.success(true, 'Credentials successfuly validated'));
...
users.createUser('Mandy', 'gatogordo@gmail.com', 'calle del melocotón, 69', 'feedmemoar', function(opRes) {
});
