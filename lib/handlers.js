...
handlers._orders.post = function(data, callback) {
    ...
    operations.addContext("handlers._orders.post");
    ...
    var requestData = helpers.retrieveRequestData(data, 
        {
            ...
            payloadFields:
                [
                    {fieldName:'creditcardnumber', type:'string', optional:false},
                    {fieldName:'cvc', type:'string', optional:false}
                ]
    ...
    if(requestData) {
        tokens.check(requestData.token, function(opRes) {
            if(opRes.success == true) {
                var receiptEmail = opRes.data.result.email;
                shoppingcart.get(requestData.token, function(opRes) {
                    if(opRes.success == true) {
                        orders.place(opRes.data.result, requestData.creditcardnumber,
                            requestData.cvc, receiptEmail, function(opRes) {
                                if(opRes.success == true) {
                                    
                                    shoppingcart.create(requestData.token, function(opRes) {
                                    
                                        if(opRes.success == true) {
                                            callback(200, {'result':'Order placed successfully'});
                                        }
                                        else {
                                            callback(500, {'result':'Order placed successfully but could not clear shopping cart'});
                                        }
                                    });
                        ...
                        callback(500, {'result':'Shopping cart not found'})
                    }
                })
            }
            else {
                callback(403, {'result':'Token not found'})
            }
        });
        
    }
    else {
        callback(400, {'result':'Bad request. Token required in header or data missing'});
    }
...
handlers._orders.get = function(data, callback) {
    ...
    operations.addContext("handlers._orders.get");
    ...
    if(requestData) {
        ...
        callback(400, {'result':'Bad request. Token required in header'});
    }
