...
handlers._tokens.post = function(data, callback) {
    ...
    if(requestData) {
        tokens.create(requestData.email, requestData.password, function(opRes) {
            if(opRes.success == true) {
                shoppingcart.create(opRes.data.result.id); // 'cannot' fail
        ...
        tokens.check(requestData.token, function(opRes) {
            if(opRes.success == true) {
                ...
                tokens.delete(tokenToDelete, function(opRes) {
                    if(opRes.success == true) {
                        var cartDestroyOpRes = shoppingcart.destroy(tokenToDelete);
                        if(cartDestroyOpRes.success == true) {
...
handlers._shoppingcart.get = function(data, callback) {
    ...
    if(requestData) {
        ...
        var shoppingCartGetOpRes = shoppingcart.get(token);
        if(shoppingCartGetOpRes.success == true) {
            callback(200, shoppingCartGetOpRes.data.result);
        }
        else {
            callback(500, {'result':shoppingCartGetOpRes.data.info});
        }
    }
    else {
        callback(400, {'result':'Bad request. Must include token in header'});
