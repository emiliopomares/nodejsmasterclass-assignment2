...
storage.listObjects = function(typeOfObject, callback) {
	operations.addContext('storage.listObjects');
...
storage.retrieveObject = function(typeOfObject, objectName, callback) {
	operations.addContext('storage.retrieveObject');
...
storage.removeObject = function(typeOfObject, objectName, callback) {
	operations.addContext('storage.removeObject')
	fs.unlink(storage.baseDir + typeOfObject + '/' + objectName + '.json', function(err) {
		if(!err) {
			callback(operations.success(true, 'Object successfully removed'));
		}
		else {
			callback(operations.error('I/O error: ' + err));
		}
})
