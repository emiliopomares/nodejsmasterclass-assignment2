...
shoppingcart.update = function(token, newItemListWithQuantities) {
	operations.addContext('shoppingcart.update');
	if(shoppinCarts[token] == undefined) {
		return operations.error('Shopping cart for token ' + token + ' does not exist');
	}
	else {
		var newCart = new ShoppingCart();
		for(var i = 0; i < newItemListWithQuantities; ++i) {
			...
			if(opRes.success) {
				...
				return operations.error('Item ' + newItemListWithQuantities[i] + ' is not on the menu');
			}
		}
		newCart.
		shoppingCarts[token] = newCart;
		return operations.success(true, 'Shopping cart updated successfully');
	}
}
